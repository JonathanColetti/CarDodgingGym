...
class CarGameEnv(gym.Env):
    ...
    The agent controls a car and must avoid colliding with static vehicle.
    ...
    def __init__(self, render_mode=None):
        ...
        self.observation_space = spaces.Box(
            ...
            high=np.array([1, 1, 1.5]), 
            dtype=np.float32
        )

        original_car = pygame.image.load("assets/cars/car.png")
        self.car = pygame.transform.scale(original_car, (int(original_car.get_width() * 0.8), int(original_car.get_height() * 0.8)))
        original_car2 = pygame.image.load("assets/cars/otherCar.png")
        self.car2 = pygame.transform.scale(original_car2, (int(original_car2.get_width() * 0.8), int(original_car2.get_height() * 0.8)))
    ...
    def _get_obs(self):
        ...
        # Normalize enemy car's to zero or oneee
        enemy_y_norm = self.car2_loc.center[1] / self.SCREEN_HEIGHT
        return np.array([player_lane_obs, enemy_lane_obs, enemy_y_norm], dtype=np.float32)
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.np_random.integers(0, 2) == 0:
            self.car_loc = self.car.get_rect(center=(self.left_lane, self.SCREEN_HEIGHT * 0.85))
        else:
            self.car_loc = self.car.get_rect(center=(self.right_lane, self.SCREEN_HEIGHT * 0.85))

        if self.np_random.integers(0, 2) == 0:
            self.car2_loc = self.car2.get_rect(center=(self.left_lane, -self.car2.get_height()))
        else:
            self.car2_loc = self.car2.get_rect(center=(self.right_lane, -self.car2.get_height()))
    ...
    def step(self, action):
        ...
        # if left
        if action == 0: 
            if self.car_loc.centerx == self.right_lane:
                self.car_loc.centerx = self.left_lane
                reward += PENALTY_LANE_CHANGE
        # if right
        elif action == 2:
            if self.car_loc.centerx == self.left_lane:
    ...
    def _render_frame(self):
        ...
        pygame.draw.rect(canvas, self.DARK_ROAD_COLOR, (self.SCREEN_WIDTH/2 - self.road_w/2, 0, self.road_w, self.SCREEN_HEIGHT))
        pygame.draw.rect(canvas, self.WHITE_LINE_COLOR, (self.SCREEN_WIDTH/2 - self.road_w/2 + self.roadmark_w*2, 0, self.roadmark_w, self.SCREEN_HEIGHT))
        pygame.draw.rect(canvas, self.WHITE_LINE_COLOR, (self.SCREEN_WIDTH/2 + self.road_w/2 - self.roadmark_w*3, 0, self.roadmark_w, self.SCREEN_HEIGHT))

        self.line_offset = (self.line_offset + self.speed) % (self.SCREEN_HEIGHT / 10)
        for y in range(-int(self.SCREEN_HEIGHT/10), self.SCREEN_HEIGHT, int(self.SCREEN_HEIGHT/10)):
             pygame.draw.rect(canvas, self.YELLOW_LINE_COLOR, (self.SCREEN_WIDTH/2 - self.roadmark_w/2, y + self.line_offset, self.roadmark_w, self.SCREEN_HEIGHT/20))
